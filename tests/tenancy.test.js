...
describe('tenant isolation', () => {
  ...
  beforeAll(async () => {
    ...
    vendorId = (await request(app).post('/vendors').set(as(a.procurement)).send({ name: 'A-only vendor' })).body.id;
    prId = (await request(app).post('/purchase-requests').set(as(a.requester))
      .send({ title: 'A-only', items: [{ description: 'x', quantity: 1, unitPrice: 10 }] })).body.id;
    await request(app).post(`/purchase-requests/${prId}/submit`).set(as(a.requester));
    await request(app).post(`/purchase-requests/${prId}/approve`).set(as(a.approver));
    poId = (await request(app).post('/purchase-orders').set(as(a.procurement)).send({ prId, vendorId })).body.id;
  });

  it('tenant A can see its own data', async () => {
    expect((await request(app).get(`/vendors/${vendorId}`).set(as(a.procurement))).status).toBe(200);
    expect((await request(app).get(`/purchase-orders/${poId}`).set(as(a.procurement))).status).toBe(200);
  });

  it('tenant B gets 404 for every tenant A id', async () => {
    const attempts = [
      request(app).get(`/vendors/${vendorId}`).set(as(b.procurement)),
      request(app).patch(`/vendors/${vendorId}`).set(as(b.procurement)).send({ name: 'hijack' }),
      request(app).delete(`/vendors/${vendorId}`).set(as(b.procurement)),
      request(app).get(`/purchase-requests/${prId}`).set(as(b.approver)),
      request(app).patch(`/purchase-requests/${prId}`).set(as(b.admin)).send({ title: 'hijack' }),
      request(app).post(`/purchase-requests/${prId}/approve`).set(as(b.approver)),
      request(app).get(`/purchase-orders/${poId}`).set(as(b.procurement)),
    ];
  ...
  it('tenant B cannot cancel A\'s order or build one from A\'s request and vendor', async () => {
    expect((await request(app).post(`/purchase-orders/${poId}/cancel`).set(as(b.procurement))).status).toBe(409);
    const res = await request(app).post('/purchase-orders').set(as(b.procurement)).send({ prId, vendorId });
    expect(res.status).toBe(404);
  });

  it('tenant A\'s data is untouched after B\'s attempts', async () => {
    const vendor = await request(app).get(`/vendors/${vendorId}`).set(as(a.procurement));
    expect(vendor.body.name).toBe('A-only vendor');
    expect(vendor.body.status).toBe('ACTIVE');
    const po = await request(app).get(`/purchase-orders/${poId}`).set(as(a.procurement));
  ...
  it('lists never include another tenant\'s rows', async () => {
    const [vendors, requests, orders] = await Promise.all([
      request(app).get('/vendors?pageSize=100').set(as(b.procurement)),
      request(app).get('/purchase-requests?pageSize=100').set(as(b.approver)),
      request(app).get('/purchase-orders?pageSize=100').set(as(b.procurement)),
    ]);
  ...
  it('login is per tenant: same email, different tenant, different user', async () => {
    const me = async (token) => (await request(app).get('/auth/me').set(as(token))).body;
  ...
  it('register creates the user in the admin\'s own tenant', async () => {
    ...
    const created = await request(app).post('/auth/register').set(as(b.admin))
      .send({ name: 'New', email, password: 'Password123!', role: 'REQUESTER' });
    expect(created.status).toBe(201);
    const ma = (await request(app).get('/auth/me').set(as(b.admin))).body;
    expect(created.body.tenantId).toBe(ma.tenantId);
    // The user cannot log in to the other tenant.
    const other = await request(app).post('/auth/login').send({ tenant: 'acme', email, password: 'Password123!' });
  ...
  it('rejects tokens without a tenant claim', async () => {
    ...
    expect((await request(app).get('/vendors').set(as(old))).status).toBe(401);
  });
...
describe('platform tenant onboarding', () => {
  ...
  it('requires the platform key', async () => {
    expect((await request(app).post('/platform/tenants').send(body())).status).toBe(401);
    expect((await request(app).post('/platform/tenants').set('x-platform-key', 'wrong').send(body())).status).toBe(401);
  });

  it('creates a tenant and its admin, who can log in and sees nothing from other tenants', async () => {
    const payload = body();
    const res = await request(app).post('/platform/tenants').set('x-platform-key', 'test-platform-key').send(payload);
    expect(res.status).toBe(201);
    expect(res.body.admin.role).toBe('ADMIN');

    const token = await login('boss@example.com', payload.slug);
    expect(token).toBeTruthy();
    const vendors = await request(app).get('/vendors').set(as(token));
    expect(vendors.body.total).toBe(0);
  });

  it('rejects a duplicate slug and a bad slug', async () => {
    const payload = body();
    await request(app).post('/platform/tenants').set('x-platform-key', 'test-platform-key').send(payload);
    const dup = await request(app).post('/platform/tenants').set('x-platform-key', 'test-platform-key').send(payload);
    expect(dup.status).toBe(409);
    const bad = await request(app).post('/platform/tenants').set('x-platform-key', 'test-platform-key')
